...
#include "../modules/gmysqlbackend/wmutility.hh"
/* to do:
...
static std::string random_element_from_vector(const std::vector<std::string>& data)
{
  ...
  std::random_device rd; // 随机数设备，用来生成种子
  std::mt19937 gen(rd()); // 使用Mersenne Twister算法的随机数生成器
  std::uniform_int_distribution<> distr(0, data.size() - 1); // 均匀分布的随机数生成器

  int random_index = distr(gen); // 生成随机数
